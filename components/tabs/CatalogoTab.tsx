...
// Helper to calculate total & unit cost of a recipe based on master ingredients
export function calculateRecipeCost(recipe: Recipe, masterIngredients: IngredientMaster[]): { totalCost: number; unitCost: number; servings: number } {
  let totalCost = 0

  recipe.ingredients.forEach(ing => {
    if (!ing.name) return
    const master = masterIngredients.find(m => m.name.toLowerCase().trim() === ing.name.toLowerCase().trim())
    ...
    if (match) {
      ...
      if (master && master.package_size > 0) {
        let costPerUnit = master.package_cost / master.package_size
  ...
  return { totalCost, unitCost: Math.round(unitCost), servings }
}
